...
var distpickerInit = function(authorities) {
    province = getUrlParam("p") != null ? getUrlParam("p") : 0;
    city = getUrlParam("c") != null ? getUrlParam("c") : 0;
    district = getUrlParam("d") != null ? getUrlParam("d") : 0;

    areaCode = district ? district : (city ? city : province);

    $.each(authorities, function(key, value) {
        if (value.authority.substring(0, 4) == "AREA") {
